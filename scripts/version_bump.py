...
def new_version_to_pre_install_js(old_version: str, new_version: str) -> None:
    with open("pre-install.js", "r") as f:
        content = f.read()
    old_v_log = f"Installing and compiling codemd {old_version}"
    old_v_install = f"cargo install codemd --vers {old_version}"
    new_v_log = f"Installing and compiling codemd {new_version}"
    new_v_install = f"cargo install codemd --vers {new_version}"
